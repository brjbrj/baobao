...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">大宝贝助手</h3>
	</div>
...
<div class="panel panel-primary">
	<div class="panel-heading">
		<h3 class="panel-title" align="center">数据库及相关配置</h3>
	</div>
<div class="progress progress-striped">
...
echo <<<HTML
		...
		<input type="text" class="form-control"placeholder="开头带上http:///(具体到安装包解压所在目录)" name="domain">
		<label for="name">上传目录:</label>
		<input type="text" class="form-control" placeholder="upload(默认)，如要更改请将upload里的do.php移至存放文件的目录下" name="file">
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from ".$dbconfig['dbqz']."_user where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过大宝贝助手</div>
	...
	$db_qz=isset($_POST['db_qz'])?$_POST['db_qz']:'tool';
	$domain=isset($_POST['domain'])?$_POST['domain']:NULL;
	$file=isset($_POST['file'])?$_POST['file']:NULL;
	if($db_host==null || $db_port==null || $db_user==null || $db_pwd==null || $db_name==null || $db_qz==null|| $domain==null|| $file==null){
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if(DB::query("select * from ".$db_qz."_user where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过大宝贝助手</div>
				<div class="list-group-item">
...
<?php
	...
	echo '<div class="alert alert-info"><font color="green">安装完成！</font><br/><br/><a href="../">>>网站首页</a><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 install.lock 文件！</font></div>';
?>
	</div>
</div>
